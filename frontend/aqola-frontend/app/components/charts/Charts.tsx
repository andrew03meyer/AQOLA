...
export default function Charts() {
  ...
  const prevChartAreasRef = useRef<Record<string, string[]>>(
    Object.fromEntries(
      getOpenCharts().map((chart) => [chart.chartName, chart.selectedAreas]),
    ),
  );
  ...
  useEffect(() => {
    ...
    const addedCharts = openCharts.filter(
      (c) => !prevRenderedCharts.has(c.chartName),
    );

    let changedCharts: StateDefinition[] = [];
    if (openCharts.length > 0) {
      changedCharts = openCharts.filter((chart) => {
        const prevAreas = prevChartAreasRef.current[chart.chartName];
        return (
          JSON.stringify(prevAreas) !== JSON.stringify(chart.selectedAreas)
        );
      });
    }
    ...
    const removedCharts = [...prevRenderedCharts].filter(
      (chart) => !currentCharts.has(chart),
    );
    ...
    if (chartsToUpdate.size > 0) {
      Promise.all(
        ...
        setChartsData((prevChartData) => ({
          ...prevChartData,
          ...Object.fromEntries(chartDataToAdd),
        }));
      });
    }
        ...
        return (
          <ChartWindow
            ...
            zIndex={
              400 +
              openCharts.length -
              openCharts.findIndex((c) => c.chartName === chart.chartName)
            }
          />
        );
      })}
